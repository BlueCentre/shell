...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		metallbEnabled := pulumiConf.GetBool("metallb_enabled", false)
		...
		// Deploy MetalLB first (if enabled) to provide LoadBalancer support
		// This is needed for bare-metal/kind/minikube clusters before deploying services that need LoadBalancers
		if metallbEnabled {
			if err := applications.DeployMetalLB(ctx, k8sProvider); err != nil {
				return err
			}
		}
