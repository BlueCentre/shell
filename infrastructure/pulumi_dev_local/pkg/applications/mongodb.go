...
func DeployMongoDB(ctx *pulumi.Context, provider *kubernetes.Provider) (*ApplicationDeployment, error) {
	...
	// Create the MongoDB namespace explicitly
	mongoNamespace, err := resources.CreateK8sNamespace(ctx, provider, resources.K8sNamespaceConfig{
		Name: namespace,
	})
	if err != nil {
		return nil, err
	...
	mongodbOperatorCRDs, err := resources.DeployHelmChart(ctx, provider, resources.HelmChartConfig{
		...
		Namespace:       namespace,
		CreateNamespace: false, // Changed to false since we created it explicitly
		// No specific values needed for CRDs chart usually
	}, pulumi.DependsOn([]pulumi.Resource{mongoNamespace}))
	if err != nil {
		return nil, err
	}

	// Create MongoDB password secret *after* namespace is created, but *before* operator chart.
	...
	}, pulumi.Provider(provider), pulumi.DependsOn([]pulumi.Resource{mongoNamespace, mongodbOperatorCRDs}))
	if err != nil {
		return nil, err
	}
