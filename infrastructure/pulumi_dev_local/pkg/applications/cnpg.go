...
func DeployCnpgCluster(ctx *pulumi.Context, provider *kubernetes.Provider, operatorRelease pulumi.Resource) (pulumi.Resource, error) {
	...
	// --- Namespace Creation --- //
	// Create the cluster namespace explicitly before creating any resources in it
	clusterNs, err := resources.CreateK8sNamespace(ctx, provider, resources.K8sNamespaceConfig{
		Name: clusterNamespace,
	})
	if err != nil {
		return nil, fmt.Errorf("failed to create CNPG cluster namespace: %w", err)
	}
	...
	}, pulumi.Provider(provider), pulumi.DependsOn([]pulumi.Resource{clusterNs, operatorRelease})) // Depend on namespace and operator
	if err != nil {
	...
	clusterRelease, err := resources.DeployHelmChart(ctx, provider, resources.HelmChartConfig{
		...
		CreateNamespace: false,                                     // Changed to false - we created it explicitly
