...
variable "metallb_enabled" {
  description = "Enable MetalLB for LoadBalancer support in kind/bare-metal clusters"
  type        = bool
  default     = false
}

variable "metallb_version" {
  description = "Version of MetalLB Helm chart to deploy"
  type        = string
  default     = "0.14.9"
}

variable "metallb_ip_range" {
  description = "IP address range for MetalLB to allocate LoadBalancer IPs (e.g., '172.18.255.200-172.18.255.250' for kind)"
  type        = string
